...
def multiclass_batch(class_count, batches, batch_size):
    """Figures out how many images of each class should be in each batch to make contents consistent as possible"""
    ...
    if path.exists('./mirabest_batch_list.p') is False:
        ...
        if sum(class_count)==(batches*batch_size): 
            ...
            # Any over/underflow in length should go into the test batch; this will keep track of how many should go in
            extra_count = sum(class_count) - (batches*batch_size)

        #---------------------

        test_count = np.floor(class_count/batches) # Divide the class vector by the no. of batches and floor it
        test_count[np.where(test_count == 0)] = 1  # If any entry equals zero, set it to one instead

        if sum(test_count) == batch_size and extra_count == 0: # Case where the number of test entries divides by batch size
        ...
        elif sum(test_count) < batch_size: # Case when there's not enough entries
            ...
            if to_fill <= len_class: # If there are fewer entries to be filled than there are classes
            ...
            # If number to fill divides neatly by the number of classes, test_count just gets that number added to every entry
